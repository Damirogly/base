...
pub struct TestConfig {
    ...
    /// If `true`, every transaction targets a freshly derived recipient
    /// signing key generated by the same derivation as the sender pool
    /// (mnemonic when set, otherwise `seed`). This forces account-trie
    /// fan-out for payloads that honor the runner-supplied `to` address.
    ///
    /// Recipient keys are advanced past the sender keys, so you can recover
    /// recipients out-of-band via
    /// `AccountPool::from_mnemonic(mnemonic, n, sender_offset + sender_count)`
    /// or `AccountPool::with_offset(seed, n, sender_offset + sender_count)`.
    /// The runner prints `recipient_offset` at startup.
    #[serde(default)]
    pub fresh_recipients: Option<bool>,
...
impl fmt::Debug for TestConfig {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("TestConfig")
            ...
            .field("fresh_recipients", &self.fresh_recipients)
    ...
    pub fn to_summary(&self) -> ConfigSummary {
        ConfigSummary {
            ...
            transactions: serde_json::to_value(&self.transactions)
                ...
                .unwrap_or_default(),
            fresh_recipients: self.fresh_recipients.unwrap_or(false),
...
"#;
    ...
    #[test]
    fn fresh_recipients_defaults_to_none() {
        let yaml = r#"
transaction_submission_rpcs: http://localhost:8545
flashblocks_ws: ws://localhost:7111
"#;
        let config = TestConfig::from_yaml(yaml).unwrap();
        assert!(config.fresh_recipients.is_none());
        assert!(!config.to_load_config(Some(1337)).unwrap().fresh_recipients);
    }

    #[test]
    fn fresh_recipients_explicit_true_round_trips() {
        let yaml = r#"
transaction_submission_rpcs: http://localhost:8545
flashblocks_ws: ws://localhost:7111
...
"#;
        let config = TestConfig::from_yaml(yaml).unwrap();
        assert_eq!(config.fresh_recipients, Some(true));
        assert!(config.to_load_config(Some(1337)).unwrap().fresh_recipients);
        assert!(config.to_summary().fresh_recipients);
    }

    #[test]
    fn fresh_recipients_explicit_false_round_trips() {
        let yaml = r#"
transaction_submission_rpcs: http://localhost:8545
flashblocks_ws: ws://localhost:7111
...
"#;
        let config = TestConfig::from_yaml(yaml).unwrap();
        assert_eq!(config.fresh_recipients, Some(false));
        assert!(!config.to_load_config(Some(1337)).unwrap().fresh_recipients);
    }
